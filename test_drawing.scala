...
object test_drawing {
  ...
  def draw(): Unit = {
    ...
    val photo = javax.imageio.ImageIO.read(new java.io.File("drawing.png"))
    val photo_g = photo.createGraphics()

    photo_g.setRenderingHint(java.awt.RenderingHints.KEY_ANTIALIASING,
                       java.awt.RenderingHints.VALUE_ANTIALIAS_ON)

    // draw two filled circles
    photo_g.setColor(Color.CYAN)
    photo_g.fillRect(40, 40, 40, 40)

    photo_g.dispose()

    // write image to a file
    javax.imageio.ImageIO.write(photo, "png", new java.io.File("final.png"))
